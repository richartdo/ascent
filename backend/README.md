...
CV analysis uses transparent quick local processing for long inputs. FastAPI removes contact details and selects a bounded representative excerpt before Ollama generation; the public response includes `inputCoverage.mode`, `originalCharacters`, and `analyzedCharacters` so clients can clearly distinguish full-text review from excerpt-based guidance.
